        ...
        if(RECOMMEND_PROGRAM>0){
            ...
            lifecycleScope.launch(Dispatchers.IO){
                ...
                lifecycleScope.launch{
                    for(i in 1..division)
                        tab_layout.addTab(tab_layout.newTab().setCustomView(createTabView("${i}일차",applicationContext)))
                    view_pager.adapter = ContentsPagerAdapter(this@ProgramInfoActivity, tab_layout.tabCount, ID,ContentsPagerAdapter.CUSTOM_PROGRAM_ACTIVITY)
                }
            }
        }
